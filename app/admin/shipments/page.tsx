...
export default async function AdminShipmentsPage({ searchParams }: Props) {
  ...
  const filteredShipments = shipments.filter((shipment) => {
    ...
    return shipment.status === activeStatus;
  });
  ...
  const tabs = [
    { label: "All", value: "all", count: shipments.length },
    {
      label: "Packing",
      value: "REQUESTED",
      count: shipments.filter((s) => s.status === "REQUESTED").length,
    },
    {
      label: "Payment",
      value: "AWAITING_PAYMENT",
      count: shipments.filter((s) => s.status === "AWAITING_PAYMENT").length,
    },
    {
      label: "Shipped",
      value: "SHIPPED",
    ...
    <main className="p-4 md:p-8">
      <section className="rounded-3xl border bg-white p-6">
        ...
        <p className="mt-2 text-gray-600">
          Price shipments, review payments, add tracking numbers, and update
          delivery progress.
        </p>
      </section>
      ...
      <form className="mt-6 rounded-3xl border bg-white p-5">
        <input
          ...
          placeholder="Search shipment ID, customer, parcel tracking, carrier..."
